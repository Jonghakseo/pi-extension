...
- 자유 입력/기타 입력에서 `@`로 파일 경로 자동완성 (`fd` 설치 시 퍼지 검색)
- 입력 유실 방지: 응답을 입력한 상태에서 Esc를 누르면 바로 닫지 않고 한 번 더 눌러 확인
...
UI를 쓸 수 없는 환경(헤드리스 실행 등)에서는 에러 대신 질문 목록을 텍스트로 반환합니다. 에이전트는 이 목록을 보고 사용자에게 일반 텍스트로 직접 물어볼 수 있습니다. 이때 `details`에는 정규화된 질문 목록과 함께 `cancelled: true`가 내려갑니다.
