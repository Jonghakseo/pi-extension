...
function createDelayController(pi: ExtensionAPI) {
	...
	function submitTask(task: DelayTask, reason: SubmitReason): boolean {
		...
		try {
			...
			pi.sendUserMessage(task.prompt, idle ? undefined : { deliverAs: "followUp" });
			if (task.ctx.hasUI) {
				const message =
					reason === "manual"
						? idle
...
export default function (pi: ExtensionAPI) {
	const controller = createDelayController(pi);
	pi.registerCommand("delay", {
		...
		handler: async (args, ctx) => {
			...
			const message = await controller.handleDelayCommand(args ?? "", ctx);
			ctx.ui.notify(
				message,
	...
	pi.registerCommand("delay-list", {
		...
		handler: async (_args, ctx) => {
			controller.setLatestContext(ctx);
			await controller.handleDelayListCommand(ctx);
		},
	});
	...
	pi.registerCommand("delay-cancel", {
		...
		getArgumentCompletions: (prefix) => {
			...
			const items = [
				{ value: "all", label: "all" },
				...controller.getTaskIds().map((id) => ({ value: id, label: id })),
			].filter((item) => item.value.startsWith(trimmed));
			return items.length > 0 ? items : null;
		},
		handler: async (args, ctx) => {
			controller.setLatestContext(ctx);
			const message = controller.handleDelayCancelCommand(args ?? "");
	...
	pi.registerTool({
		...
		async execute(_toolCallId, rawParams, _signal, _onUpdate, ctx) {
			...
			const task = controller.scheduleDelay(ctx, delayMs, params.prompt.trim(), params.id);
			return {
				content: [
					{
						type: "text" as const,
		...
		renderCall(args, theme) {
			...
			return new Text(
				`${theme.fg("toolTitle", theme.bold("delay "))}${theme.fg("accent", params.delay ?? "?")} ${theme.fg("dim", controller.preview(params.prompt ?? ""))}`,
				0,
				0,
			);
		},
